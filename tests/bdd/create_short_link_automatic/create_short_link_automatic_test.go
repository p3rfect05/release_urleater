package create_short_link_automatic
...
func iChooseAutomaticGeneration(ctx context.Context) error {
	driver := ctx.Value("driver").(selenium.WebDriver)

	elem, err := driver.FindElement(selenium.ByID, "autoGenerate")
...
func iGetNewlyGeneratedLink(ctx context.Context) error {
	driver := ctx.Value("driver").(selenium.WebDriver)
	...
	matched, err := regexp.MatchString("http://localhost:8080/.+", text)
	if err != nil {
		panic(err)
	}
	if !matched {
...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.When(`^I type (.*)$`, iTypeLongUrl)
	ctx.When(`^I choose automatic generation$`, iChooseAutomaticGeneration)
	ctx.When(`^I click on create button$`, iClickOnCreateButton)
	ctx.Then(`^I get newly generated Link$`, iGetNewlyGeneratedLink)
